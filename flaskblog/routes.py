...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            next_page = request.args.get('next') #args is dictionary but we use get cause [] can raise error
...
def save_picture(form_picture):
    ...
    picture_path = os.path.join(app.root_path,'static/profile_pics',picture_fn) # odredjivanje putanje slike, gde ce se nalaziti, join omogucava da se to lepo odradi
...
@app.route("/post/<int:post_id>")#naznacavamo da je int
def post(post_id):
    post = Post.query.get_or_404(post_id) #get the post, if it doesnt exist return 404 (page does not exist)
    return render_template('post.html',title=post.title,post=post)

@app.route("/post/<int:post_id>/update",methods=['GET','POST'])#naznacavamo da je int
@login_required
...
@app.route("/post/<int:post_id>/delete",methods=['POST'])#naznacavamo da je int 
@login_required
